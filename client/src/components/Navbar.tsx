...
export default function Navbar() {
    ...
    const isAdmin = isLoggedIn && user?.email?.toLowerCase() === import.meta.env.VITE_ADMIN_EMAIL?.toLowerCase();
    ...
    return (
        <>
            ...
            >
                <Link to={isAdmin ? '/admin/messages' : '/'}>
                <img src="/logo.svg" alt="Logo" className="h-8.5 w-auto" />
                </Link>

                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    {isAdmin ? (
                        <Link to='/admin/messages' className="hover:text-pink-500 transition">Admin</Link>
                    ) : (
                        <>
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
               {isAdmin ? (
                <Link onClick={() => setIsOpen(false)} to='/admin/messages'>Admin</Link>
               ) : (
                <>
