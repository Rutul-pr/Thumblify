...
const MyGeneration = () => {
  ...
  const formatThumbnailDate = (thumb: IThumbnail) => {
    const rawDate = thumb.createdAt || thumb.updatedAt;
  ...
  return (
    <>
        ...
        <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
          ...
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-zinc-200 ">My Generations
            </h1>
                ...
                return (
                  <div key={thumb._id} onClick={()=>navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid ">
                    ...
                    <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                      {thumb.image_url ? (
                        <img src={thumb.image_url} alt="Thumbnail Image" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-sm text-zinc-400">
                          {thumb.isGenerating ? 'Generating...' : 'No Image Now'}
                        </div>
                      )}
                    ...
                    <div className="p-4 space-y-2">
                      ...
                      <p className="text-xs text-zinc-500">{formatThumbnailDate(thumb)}</p>
                    </div>
